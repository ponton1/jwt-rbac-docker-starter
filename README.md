...
A production-ready modular authentication API built with Node.js,
Express, and PostgreSQL.

Implements secure JWT authentication (access + refresh), refresh token
rotation, immediate global revocation using tokenVersion, and Role-Based
Access Control (RBAC).

Designed as a clean backend foundation ready for Docker, CI/CD, testing,
...
4.  Refresh rotation replaces old token and marks it revoked.
5.  logout() revokes a single refresh token.
6.  logoutAll() increments tokenVersion and revokes all refresh tokens.
7.  Any access token with outdated tokenVersion becomes invalid
    immediately.
